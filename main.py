...
def explore(tree:ns.Node,indent:str=''):
    ...
    if isinstance(tree,ns.Node):
        ...
        for n in tree.buffer if type(tree) == ns.NodeExpression else [tree.node] if type(tree) == ns.NodeAccessDot else [tree.value,tree.index] if type(tree) == ns.NodeIndex else [tree.value,*tree.args] if type(tree) == ns.NodeCall else [tree.name,tree.hint,*tree.children] if type(tree) == ns.NodeLet else tree.children:
            explore(n,indent+'    ')
    else:
